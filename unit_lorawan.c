...
#define UNIT_LORAWAN_HEX_MESSAGE_MAX_SIZE   512  // Allows up to 256 bytes payload, doubled for hex encoding
...
static esp_err_t _unit_lorawan_send_at_command( const char * cmd , lorawan_response_t * response , uint32_t timeout_ms ) ;
static esp_err_t _unit_lorawan_parse_response( const char * raw_response , size_t response_len , lorawan_response_t * parsed_response ) ;
static esp_err_t _unit_lorawan_wait_for_response( char * buffer , size_t buffer_size , size_t * received_len , uint32_t timeout_ms ) ;
static esp_err_t _unit_lorawan_validate_payload_size( size_t payload_size , uint8_t data_rate ) ;
static void _unit_lorawan_cleanup_response( lorawan_response_t * response ) ;
static const char * _unit_lorawan_get_connection_status_description( const char * status_code ) ;
...
static esp_err_t _unit_lorawan_validate_payload_size( size_t payload_size , uint8_t data_rate )
{
    ...
    switch( data_rate )
    {
        case 0: max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR0; break;
        case 1: max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR1; break;
        case 2: max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR2; break;
        case 3: max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR3; break;
        case 4: max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR4; break;
        default:
            ESP_LOGW( _TAG, "Unknown data rate %d, using maximum safe payload size", data_rate );
            max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR0;
            break;
    }
    if( payload_size > max_payload )
    {
        ESP_LOGE( _TAG, "Payload size %zu exceeds maximum %zu bytes for data rate DR%d", 
...
static esp_err_t _unit_lorawan_wait_for_response( char *buffer, size_t buffer_size, size_t *received_len, uint32_t timeout_ms )
{
    *received_len = 0;
    ...
    while( ( xTaskGetTickCount() * portTICK_PERIOD_MS - start_time ) < timeout_ms )
    {
        ...
        esp_err_t err = core2foraws_expports_uart_read( ( uint8_t* ) buffer, &available_bytes );
...
static esp_err_t _unit_lorawan_parse_response( const char *raw_response, size_t response_len, lorawan_response_t *parsed_response )
{
    ...
    memset( parsed_response->error_code, 0, sizeof(parsed_response->error_code) );
    ...
    if( strstr( raw_response, "OK" ) != NULL )
    {
            ...
            strstr( raw_response, "+CJOIN:" ) != NULL )
        {
            ...
            if( parsed_response->response_data )
            {
                ...
                ESP_LOGD( _TAG, "Response data captured: %s", parsed_response->response_data );
            }
        }
    }
    ...
    else if( strstr( raw_response, "ERROR" ) != NULL )
    {
        ...
        ESP_LOGW( _TAG, "Command failed with error: %s", parsed_response->error_code );
    }
    // Check for specific LoRaWAN responses with +COMMAND: format (fallback for responses without OK)
...
static esp_err_t _unit_lorawan_send_at_command( const char *cmd, lorawan_response_t *response, uint32_t timeout_ms )
{
    ...
    // Format AT command
    size_t at_cmd_len = strlen( cmd ) + 6;  // "AT+" + cmd + "\r\n" + null terminator
    ...
    for( int retry = 0; retry < UNIT_LORAWAN_MAX_RETRIES; retry++ )
    {
        if( retry > 0 )
        {
            ESP_LOGW( _TAG, "Retrying command (attempt %d/%d): %s", retry + 1, UNIT_LORAWAN_MAX_RETRIES, cmd );
        ...
        err = _unit_lorawan_wait_for_response( response_buffer, UNIT_LORAWAN_RESPONSE_BUFFER_SIZE - 1, 
                                            &received_len, timeout_ms );
        
        if( err == ESP_OK && response )
        {
            err = _unit_lorawan_parse_response( response_buffer, received_len, response );
        }
    ...
    if( err != ESP_OK )
    {
        ESP_LOGE( _TAG, "Command failed after %d retries: %s", UNIT_LORAWAN_MAX_RETRIES, cmd );
    }
...
esp_err_t unit_lorawan_log( uint8_t level )
{
    ...
    esp_err_t err = _unit_lorawan_send_at_command( log_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
...
esp_err_t unit_lorawan_connected( bool *state )
{
    ...
    *state = false;
    ...
    lorawan_response_t response = {0};
    esp_err_t err = _unit_lorawan_send_at_command( "CSTATUS?", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    if( err == ESP_OK && response.success && response.response_data )
    {
        ...
        if( status_pos )
        {
            ...
            {
                if( strstr( status_code, "04" ) != NULL || strstr( status_code, "08" ) != NULL )
                {
                    *state = true;
                ...
                {
                    const char* description = _unit_lorawan_get_connection_status_description( status_code );
                    ESP_LOGI( _TAG, "✗ Device not connected to network - Status: %s (%s)", status_code, description );
                }
            }
            else
            {
        ...
        {
            ESP_LOGW( _TAG, "✗ Could not find +CSTATUS: in response: %s", response.response_data );
        }
    }
    else
    {
...
esp_err_t unit_lorawan_join( void )
{
    ...
    esp_err_t err = _unit_lorawan_send_at_command( "CJOIN=1,1,10,8", &response, 30000 );
    
    if( err == ESP_OK && response.success )
    {
        ESP_LOGI( _TAG, "✓ LoRaWAN join command sent successfully" );
        ESP_LOGI( _TAG, "  Join process initiated - this may take up to 30 seconds" );
...
esp_err_t unit_lorawan_configOTTA( char *devEUI, char *appEUI, char *appKey, unit_lorwan_uldlmode mode )
{
    ...
    ESP_LOGI( _TAG, "Configuring LoRaWAN device for OTAA (Over-The-Air Activation)" );
    ESP_LOGI( _TAG, "  DevEUI: %s", devEUI );
    ESP_LOGI( _TAG, "  AppEUI: %s", appEUI );
    ESP_LOGI( _TAG, "  Mode: %s", ( mode == DIFFERENT_FREQ_MODE ) ? "Different frequency" : "Same frequency" );
    
    lorawan_response_t response = { 0 };
    esp_err_t err = ESP_OK;
    
    // Step 1: Set join mode to OTAA
    err = _unit_lorawan_send_at_command( "CJOINMODE=0", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    err = _unit_lorawan_send_at_command( deveui_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    err = _unit_lorawan_send_at_command( appeui_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    err = _unit_lorawan_send_at_command( appkey_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    char uldl_cmd[ 32 ];
    snprintf( uldl_cmd, sizeof( uldl_cmd ), "CULDLMODE=%s", _unit_lorawan_uldlmode_str[mode] );
    err = _unit_lorawan_send_at_command( uldl_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    err = _unit_lorawan_send_at_command( "CCLASS=0", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    err = _unit_lorawan_send_at_command( "CWORKMODE=2", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    ESP_LOGI( _TAG, "  Note: Network-specific settings (frequency band, RX2, etc.) may need to be configured separately" );
...
esp_err_t unit_lorawan_reboot( void )
{
    ...
    lorawan_response_t response = {0};
    esp_err_t err = _unit_lorawan_send_at_command( "IREBOOT=0", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
...
esp_err_t unit_lorawan_attached( bool *state )
{
    ...
    *state = false;
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( "CGMI?", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    if( err == ESP_OK && response.success && response.response_data )
    {
        ...
        {
            ESP_LOGW( _TAG, "✗ Could not find +CGMI= in response: %s", response.response_data );
        }
    }
    else
    {
...
esp_err_t unit_lorawan_send( char *message, size_t length )
{
    ...
    uint8_t current_dr;
    size_t max_payload;
    esp_err_t dr_err = unit_lorawan_get_data_rate_info( &current_dr, &max_payload );
    if( dr_err != ESP_OK )
    {
        ESP_LOGW( _TAG, "Failed to get current data rate, using conservative validation" );
        current_dr = 0;
        max_payload = UNIT_LORAWAN_US915_MAX_PAYLOAD_DR0;
    }
    
    // Validate payload size against current data rate
    esp_err_t size_check = _unit_lorawan_validate_payload_size( length, current_dr );
    if( size_check != ESP_OK )
    {
        return size_check;
    }
    
    ESP_LOGI( _TAG, "Sending LoRaWAN message (%zu bytes) on DR%d", length, current_dr );
    ...
    {
        ESP_LOGE( _TAG, "Hex message too long: %zu bytes (max: %d)", hex_len, UNIT_LORAWAN_HEX_MESSAGE_MAX_SIZE );
        free( hex_message );
        return ESP_ERR_INVALID_SIZE;
    }
    
    char *send_cmd = malloc( strlen( "DTRX=1,2," ) + 10 + hex_len + 1 ); // Extra space for length
    if( !send_cmd )
    {
    ...
    snprintf( send_cmd, strlen( "DTRX=1,2," ) + 10 + hex_len + 1, "DTRX=1,2,%zu,%s", length, hex_message );
    
    lorawan_response_t response = {0};
    esp_err_t err = _unit_lorawan_send_at_command( send_cmd, &response, 30000 );  // Longer timeout for send
    
    if( err == ESP_OK && response.success )
    {
        ESP_LOGI( _TAG, "✓ LoRaWAN message sent successfully" );
        ESP_LOGI( _TAG, "  Message will be transmitted when network conditions allow" );
...
static esp_err_t _configure_ttn_network_parameters( const unit_lorawan_ttn_config_t *config )
{
    ...
    // RX2 parameters are automatically handled by the LoRaWAN stack per regional parameters
    ESP_LOGI( _TAG, "RX2 parameters automatically configured per US915 regional parameters" );
    
    // Enable/disable Adaptive Data Rate
    char adr_cmd[ 16 ];
    snprintf( adr_cmd, sizeof( adr_cmd ), "CADR=%d", config->adr_enabled ? 1 : 0 );
    
    err = _unit_lorawan_send_at_command( adr_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
        ESP_LOGE( _TAG, "Failed to configure ADR setting" );
        goto cleanup;
    }
    ESP_LOGI( _TAG, "✓ Adaptive Data Rate %s", config->adr_enabled ? "enabled" : "disabled" );
    _unit_lorawan_cleanup_response( &response );
    
    char dr_cmd[ 32 ];
    snprintf( dr_cmd, sizeof( dr_cmd ), "CDATARATE=%d", config->data_rate );
    
    err = _unit_lorawan_send_at_command( dr_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
        ESP_LOGW( _TAG, "Initial data rate setting failed - will rely on ADR if enabled" );
    ...
    ESP_LOGI( _TAG, "✓ Initial data rate configured (DR%d, max payload: %zu bytes)", 
...
esp_err_t unit_lorawan_set_rx2_frequency( uint32_t frequency )
{
    ...
    ESP_LOGW( _TAG, "Device uses automatic RX2 settings per LoRaWAN US915 regional parameters" );
    ESP_LOGW( _TAG, "TTN US915 RX2: 923.3 MHz, DR8 (configured automatically by stack)" );
...
esp_err_t unit_lorawan_get_channel_rssi( uint8_t freq_band_idx , int16_t * rssi_values , size_t * channel_count )
{
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    if( err == ESP_OK && response.success && response.response_data )
    {
        ...
        if( line_start )
        {
            ...
            if( line_start ) line_start++; // Move past newline
            ...
            while( line && count < 8 )
            {
                ...
                if( sscanf( line, "%d:%d", &channel, &rssi_val ) == 2 )
                {
                    ...
                    {
                        ESP_LOGW( _TAG, "Unexpected channel number %d at position %zu", channel, count );
                    }
                }
                else
                {
                ...
                line = strchr( line, '\n' );
                if( line ) line++; // Move to next line
            }
            
            if( count == 8 )
            {
                * channel_count = count;
                ESP_LOGI( _TAG, "✓ Successfully scanned RSSI for all %zu channels in frequency band %d", count, freq_band_idx );
            }
            else
            {
    ...
    {
        ESP_LOGE( _TAG, "✗ Failed to scan channel RSSI values for frequency band %d", freq_band_idx );
    }
...
esp_err_t unit_lorawan_set_data_rate( uint8_t data_rate )
{
    if( data_rate > UNIT_LORAWAN_US915_DATA_RATE_MAX )
    {
        ESP_LOGE( _TAG, "Invalid data rate %d for US915 (valid range: %d-%d)", 
                data_rate, UNIT_LORAWAN_US915_DATA_RATE_MIN, UNIT_LORAWAN_US915_DATA_RATE_MAX );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( dr_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    else
    {
        ...
        ESP_LOGW( _TAG, "Note: If ADR is enabled, network may override this setting" );
    }
...
esp_err_t unit_lorawan_set_retries( uint8_t message_type, uint8_t retries )
{
    if( message_type > 1 )
    {
        ESP_LOGE( _TAG, "Invalid message type %d (0=unconfirmed, 1=confirmed)", message_type );
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t unit_lorawan_set_tx_power( uint8_t power_index )
{
    if( power_index > 7 )
    {
        ESP_LOGE( _TAG, "Invalid TX power index %d (valid range: 0-7)", power_index );
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t unit_lorawan_get_tx_power( uint8_t *power_index )
{
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( "CTXP?", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
...
esp_err_t unit_lorawan_link_check( uint8_t mode )
{
    if( mode > 2 )
    {
        ESP_LOGE( _TAG, "Invalid link check mode %d (0=disable, 1=once, 2=after every uplink)", mode );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( cmd, &response, 
                                                  mode == 1 ? 30000 : UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    if( err == ESP_OK && response.success )
    {
        ...
        if( mode == 1 && response.response_data )
        {
            ...
            if( link_result )
            {
                ...
                if( sscanf( link_result, "+CLINKCHECK:%d,%d,%d,%d,%d", 
                          &result, &margin, &gateways, &rssi, &snr ) == 5 )
                {
                    if( result == 0 )
                    {
...
esp_err_t unit_lorawan_save_config( void )
{
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( "CSAVE", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
...
esp_err_t unit_lorawan_restore_defaults( void )
{
    ...
    lorawan_response_t response = { 0 };
    esp_err_t err = _unit_lorawan_send_at_command( "CRESTORE", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    
    if( err == ESP_OK && response.success )
    {
        ESP_LOGI( _TAG, "✓ Factory defaults restored successfully" );
        ESP_LOGI( _TAG, "  Note: You may need to reboot the module for changes to take effect" );
    }
    else
    {
                                       ...
                                       size_t response_size, uint32_t timeout_ms )
{
    ...
    esp_err_t err = _unit_lorawan_send_at_command( command, &lorawan_resp, timeout_ms );
...
esp_err_t unit_lorawan_init( void )
{
    ...
    lorawan_response_t response = { 0 };
    err = _unit_lorawan_send_at_command( "CSAVE", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err == ESP_OK && response.success )
    {
...
static const char* _get_us915_sub_band_mask( uint8_t sub_band )
{
    switch( sub_band )
    {
        case 1: return ASR6501_US915_SUB_BAND_1_MASK;
        case 2: return ASR6501_US915_SUB_BAND_2_MASK;
        case 3: return ASR6501_US915_SUB_BAND_3_MASK;
        case 4: return ASR6501_US915_SUB_BAND_4_MASK;
        case 5: return ASR6501_US915_SUB_BAND_5_MASK;
        case 6: return ASR6501_US915_SUB_BAND_6_MASK;
        case 7: return ASR6501_US915_SUB_BAND_7_MASK;
        case 8: return ASR6501_US915_SUB_BAND_8_MASK;
        default: return ASR6501_US915_SUB_BAND_2_MASK; // Default to TTN sub-band
    }
}
...
static esp_err_t _validate_ttn_config( const unit_lorawan_ttn_config_t *config )
{
    ...
    {
        ESP_LOGE( _TAG, "Invalid DevEUI length: %zu (expected %d)", strlen( config->dev_eui ), UNIT_LORAWAN_EUI_LENGTH );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        ESP_LOGE( _TAG, "Invalid AppEUI length: %zu (expected %d)", strlen( config->app_eui ), UNIT_LORAWAN_EUI_LENGTH );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        ESP_LOGE( _TAG, "Invalid AppKey length: %zu (expected %d)", strlen( config->app_key ), UNIT_LORAWAN_APP_KEY_LENGTH );
        return ESP_ERR_INVALID_ARG;
    }
    
    // Validate sub-band range (1-8 for US915)
    if( config->sub_band < UNIT_LORAWAN_US915_SUB_BAND_MIN || config->sub_band > UNIT_LORAWAN_US915_SUB_BAND_MAX )
    {
        ESP_LOGE( _TAG, "Invalid US915 sub-band: %d (valid range: %d-%d)", 
                config->sub_band, UNIT_LORAWAN_US915_SUB_BAND_MIN, UNIT_LORAWAN_US915_SUB_BAND_MAX );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if( config->data_rate > UNIT_LORAWAN_US915_DATA_RATE_MAX )
    {
        ESP_LOGE( _TAG, "Invalid US915 data rate: %d (valid range: %d-%d)", 
                config->data_rate, UNIT_LORAWAN_US915_DATA_RATE_MIN, UNIT_LORAWAN_US915_DATA_RATE_MAX );
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        ESP_LOGE( _TAG, "Invalid RX2 data rate: %d (valid range: 0-15)", config->rx2_data_rate );
        return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t _configure_us915_frequency_plan( uint8_t sub_band )
{
    ...
    err = _unit_lorawan_send_at_command( "CFREQBANDMASK=0001", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
    ...
    char channel_cmd[ 32 ];
    snprintf( channel_cmd, sizeof( channel_cmd ), "CFREQBANDMASK=%s", sub_band_mask );
    
    err = _unit_lorawan_send_at_command( channel_cmd, &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    if( err != ESP_OK || !response.success )
    {
        ESP_LOGE( _TAG, "Failed to configure US915 sub-band %d", sub_band );
        goto cleanup;
    }
    ESP_LOGI( _TAG, "✓ US915 sub-band %d configured (channels %d-%d)", 
             sub_band, ( sub_band - 1 ) * 8, ( sub_band - 1 ) * 8 + 7 );
    _unit_lorawan_cleanup_response( &response );
...
static void ttn_join_monitor_task( void *pvParameters )
{
    ...
    ESP_LOGI( _TAG, "TTN join monitoring started (timeout: %d seconds)", monitor_params->timeout_sec );
    ...
    while( elapsed_sec < monitor_params->timeout_sec )
    {
        ...
        {
            ESP_LOGI( _TAG, "✓ TTN network join successful after %d seconds", elapsed_sec );
        ...
        if( elapsed_sec % 10 == 0 )
        {
            ESP_LOGI( _TAG, "TTN join in progress... (%d/%d seconds)", elapsed_sec, monitor_params->timeout_sec );
        }
    }
    
    if( !connected )
    {
        ESP_LOGE( _TAG, "✗ TTN network join timeout after %d seconds", monitor_params->timeout_sec );
        ESP_LOGE( _TAG, "Check TTN console, gateway coverage, and credentials" );
        monitor_params->callback( false, 1, monitor_params->user_data );  // Timeout error
    }
                                          ...
                                          void *user_data )
{
    ...
    ESP_LOGI( _TAG, "  RX2: %u Hz, DR%d (configured automatically by stack)", config->rx2_frequency, config->rx2_data_rate );
    ...
    err = unit_lorawan_configOTTA( ( char* ) config->dev_eui, ( char* ) config->app_eui, 
    ...
    if( join_callback )
    {
        ...
        if( params )
        {
            ...
            xTaskCreate( ttn_join_monitor_task, "ttn_join_monitor", 4096, params, 3, NULL );
        }
        else
        {
    ...
    ESP_LOGI( _TAG, "  Join process initiated - %s", 
             join_callback ? "callback will notify of result" : "use unit_lorawan_connected() to check status" );
...
esp_err_t unit_lorawan_config_otaa_from_kconfig( void )
{
    ...
    {
        ESP_LOGE( _TAG, "Invalid CONFIG_LORAWAN_DEVICE_EUI length: %zu (expected %d)", 
    ...
    unit_lorwan_uldlmode mode = CONFIG_LORAWAN_ULDL_MODE ? SAME_FREQ_MODE : DIFFERENT_FREQ_MODE;
#else
    unit_lorwan_uldlmode mode = DIFFERENT_FREQ_MODE; // Default to different frequency (recommended for TTN)
#endif
    
    esp_err_t err = unit_lorawan_configOTTA( 
        CONFIG_LORAWAN_DEVICE_EUI,
...
#ifdef CONFIG_LORAWAN_ULDL_MODE
        ESP_LOGI( _TAG, "  Mode: %s", CONFIG_LORAWAN_ULDL_MODE ? "Same frequency" : "Different frequency" );
#else
...
esp_err_t unit_lorawan_init_with_config( unit_lorawan_ttn_join_callback_t join_callback , void * user_data )
{
    ...
    err = unit_lorawan_configure_ttn_us915( &ttn_config, join_callback, user_data );
    if( err != ESP_OK )
    {
...
#endif
#ifdef CONFIG_LORAWAN_ADR_ENABLED
    ESP_LOGI( _TAG, "  ADR: %s", CONFIG_LORAWAN_ADR_ENABLED ? "Enabled" : "Disabled" );
#endif
#ifdef CONFIG_LORAWAN_TX_POWER_INDEX
...
esp_err_t unit_lorawan_get_data_rate_info( uint8_t *current_data_rate, size_t *max_payload_size )
{
    ...
    lorawan_response_t response = {0};
    esp_err_t err = _unit_lorawan_send_at_command( "CDATARATE?", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
    ...
    if( err == ESP_OK && response.success && response.response_data )
    {
        ...
        if( cdatarate_pos )
        {
            ...
            if( sscanf( cdatarate_pos, "+CDATARATE:%d", &dr_value ) == 1 )
            {
                *current_data_rate = ( uint8_t ) dr_value;
                ...
                {
                    ESP_LOGW( _TAG, "Unknown data rate %d, using conservative payload limit", dr_value );
        ...
        {
            ESP_LOGE( _TAG, "CDATARATE data not found in response: %s", response.response_data );
            err = ESP_FAIL;
        }
    }
    else
    {
        ...
        err = _unit_lorawan_send_at_command( "CSTATUS?", &response, UNIT_LORAWAN_RESPONSE_TIMEOUT_MS );
        if( err == ESP_OK && response.success )
        {
            ESP_LOGW( _TAG, "Using default data rate DR2 due to query failure" );
            *current_data_rate = UNIT_LORAWAN_TTN_US915_DATA_RATE_DEFAULT;
            *max_payload_size = us915_max_payload_sizes[UNIT_LORAWAN_TTN_US915_DATA_RATE_DEFAULT];
        }
        else
        {
