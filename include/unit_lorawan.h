...
#define UNIT_LORAWAN_TTN_US915_RX2_FREQUENCY        923300000   ///< TTN standard RX2 frequency for US915 (Hz)
#define UNIT_LORAWAN_TTN_US915_RX2_DATA_RATE        8           ///< TTN standard RX2 data rate for US915
#define UNIT_LORAWAN_TTN_US915_SUB_BAND_DEFAULT     2           ///< TTN commonly uses sub-band 2 (channels 8-15)
#define UNIT_LORAWAN_TTN_US915_DATA_RATE_DEFAULT    2           ///< Recommended initial data rate for TTN US915
#define UNIT_LORAWAN_TTN_US915_ADR_ENABLED          true        ///< Enable ADR by default for TTN
#define UNIT_LORAWAN_TTN_US915_JOIN_TIMEOUT_SEC     60          ///< Default join timeout in seconds
#define UNIT_LORAWAN_TTN_V3_APP_EUI                 "0000000000000000"  ///< Standard AppEUI for TTN v3

// LoRaWAN US915 Data Rate Constants
#define UNIT_LORAWAN_US915_DATA_RATE_MIN            0           ///< Minimum US915 data rate (SF10, longest range)
#define UNIT_LORAWAN_US915_DATA_RATE_MAX            4           ///< Maximum US915 data rate (SF8 500kHz, shortest range)

// LoRaWAN US915 Payload Size Constants (bytes)
#define UNIT_LORAWAN_US915_MAX_PAYLOAD_DR0          11          ///< Maximum payload for DR0 (SF10, 125kHz)
#define UNIT_LORAWAN_US915_MAX_PAYLOAD_DR1          53          ///< Maximum payload for DR1 (SF9, 125kHz)
#define UNIT_LORAWAN_US915_MAX_PAYLOAD_DR2          125         ///< Maximum payload for DR2 (SF8, 125kHz)
#define UNIT_LORAWAN_US915_MAX_PAYLOAD_DR3          242         ///< Maximum payload for DR3 (SF7, 125kHz)
#define UNIT_LORAWAN_US915_MAX_PAYLOAD_DR4          242         ///< Maximum payload for DR4 (SF8, 500kHz)
...
#define UNIT_LORAWAN_EUI_LENGTH                     16          ///< Length of DevEUI and AppEUI in hex characters
#define UNIT_LORAWAN_APP_KEY_LENGTH                 32          ///< Length of AppKey in hex characters

/** 
 * @brief The maximum message size for sending LoRaWAN messages safely across all data rates.
 * 
 * This is set to the minimum payload size supported by TTN (DR0 = 11 bytes) to ensure
 * compatibility across all data rates. For higher data rates, larger payloads are supported:
 ...
 * 
 * These modes determine how the device handles uplink and downlink frequencies:
 * - DIFFERENT_FREQ_MODE: Uses different frequencies for uplink and downlink (recommended for TTN)
 * - SAME_FREQ_MODE: Uses the same frequency for both uplink and downlink
 */
typedef enum
{
    DIFFERENT_FREQ_MODE = 0,  /**< Different frequencies for uplink/downlink (TTN standard) */
 ...
 * @param error_code Error code if join failed (0 if successful, 1 for timeout)
 * @param user_data User data passed to the callback
 */
typedef void (*unit_lorawan_ttn_join_callback_t)( bool joined , uint8_t error_code , void * user_data ) ;
 ...
 *  - ESP_ERR_INVALID_ARG   : Invalid log level (will be clamped to valid range)
 */
 ...
 * 
 * Queries the module's connection status and provides human-readable status information.
 * A device must be connected to send/receive LoRaWAN messages.
 * 
 ...
 * 
 * Initiates the network join procedure using previously configured OTAA credentials.
 * The join process is asynchronous and may take up to 30 seconds to complete.
 * Use unit_lorawan_connected() to check if the join was successful.
 * 
 * @note The device must be configured with valid OTAA credentials before calling this function.
 * @note Join attempts are subject to duty cycle limitations and may be delayed.
 * 
 * @return [esp_err_t](https://docs.espressif.com/projects/esp-idf/en/release-v4.3/esp32/api-reference/system/esp_err.html#macros).
 *  - ESP_OK                : Join command sent successfully (does not guarantee successful join)
 ...
 * @param mode Upload/download frequency mode (DIFFERENT_FREQ_MODE recommended for TTN)
 * 
 * @note All EUI and key strings must be valid hexadecimal without spaces or delimiters.
 * @note This function automatically configures TTN US915 sub-band 2 (channels 8-15).
 ...
 * 
 * @note The function waits 2 seconds after sending the reboot command to allow module restart.
 * @note All network connections will be lost and must be re-established after reboot.
 * 
 * @return [esp_err_t](https://docs.espressif.com/projects/esp-idf/en/release-v4.3/esp32/api-reference/system/esp_err.html#macros).
 ...
 * The message will be automatically converted to hexadecimal format for transmission.
 * 
 * @note Message size is validated against TTN payload limits for different data rates.
 * @note The device must be connected to a network before sending messages.
 * @note Actual transmission timing depends on duty cycle limitations and network conditions.
 * 
 * @param message Pointer to the message data (ASCII format)
 * @param length Length of the message in bytes (max depends on data rate, see UNIT_LORAWAN_MAX_MESSAGE_SIZE)
 * 
 * @return [esp_err_t](https://docs.espressif.com/projects/esp-idf/en/release-v4.3/esp32/api-reference/system/esp_err.html#macros).
 ...
 * @note Detailed error messages will guide troubleshooting if initialization fails.
 * 
 * @return [esp_err_t](https://docs.espressif.com/projects/esp-idf/en/release-v4.3/esp32/api-reference/system/esp_err.html#macros).
...
/**
 * @brief Initialize and configure LoRaWAN module for The Things Network (TTN) US915
 * 
 * This function provides a complete TTN-specific configuration for the US915 frequency band
 * with all recommended settings for optimal performance and compliance with TTN requirements.
 * 
 * Key features:
 ...
 * - Sub-band: Configurable (1-8), TTN typically uses sub-band 2 (channels 8-15)
 ...
 * 
 * @param[in] config TTN configuration structure with device credentials and settings
 * @param[in] join_callback Optional callback function for asynchronous join status (can be NULL)
 * @param[in] user_data Optional user data pointer passed to join callback (can be NULL)
 * 
 * @return 
 *     - ESP_OK: TTN configuration completed successfully, join process initiated
 *     - ESP_ERR_INVALID_ARG: Invalid configuration parameters
 *     - ESP_ERR_INVALID_STATE: LoRaWAN module not initialized or not responding
 *     - ESP_FAIL: TTN configuration failed
 * 
 * @note Device credentials (DevEUI, AppEUI, AppKey) must be obtained from TTN Console
 * @note For TTN v3 stack, AppEUI should typically be all zeros: "0000000000000000"
 * @note Join process is asynchronous - use callback or unit_lorawan_connected() to check status
 ...
 * @param[out] max_payload_size Maximum payload size in bytes for current data rate
 * 
 * @return 
 ...
 * 
 * Sets the transmission data rate for LoRaWAN communications. Higher data rates
 * allow larger payloads and faster transmission but have shorter range.
 * 
 * US915 Data Rates:
 ...
 * 
 * Scans and returns RSSI values for all 8 channels in the specified frequency band.
 * Per ASR6501 datasheet, this returns RSSI values for channels 0-7 in the specified frequency group.
 * Useful for network diagnostics and channel selection.
 * 
 * @param[in] freq_band_idx Frequency band index (0-based, where 1 = group 1A2, etc.)
 * @param[out] rssi_values Array to store RSSI values (must be at least 8 elements for channels 0-7)
 * @param[out] channel_count Number of channels scanned (always 8 for valid frequency bands)
 * 
 * @return 
 ...
 * 
 * @note This function uses the AT+CRSSI FREQBANDIDX? command format per datasheet
 * @note Returns RSSI values for all 8 channels (0-7) in the specified frequency group
 */
 ...
 * 
 * Link check verifies network connectivity by requesting acknowledgment from the network server.
 * 
 ...
 * 
 * Saves all current LoRaWAN settings to flash/EEPROM so they persist after reboot.
 * 
 * @return 
 ...
 * 
 * @param[in] join_callback Optional callback function for join status (can be NULL)
 * @param[in] user_data Optional user data pointer passed to join callback (can be NULL)
 * 
 * @return 
 ...
 * @note For US915, uses CONFIG_LORAWAN_US915_SUB_BAND and CONFIG_LORAWAN_US915_DATA_RATE
 */
 ...
 * Uses CONFIG_LORAWAN_DEVICE_EUI, CONFIG_LORAWAN_APP_EUI, and CONFIG_LORAWAN_APP_KEY.
 * 
 * @return 
 *     - ESP_OK: OTAA configuration completed successfully
 ...
 * Uses CONFIG_LORAWAN_DEV_ADDR, CONFIG_LORAWAN_APP_SKEY, and CONFIG_LORAWAN_NWK_SKEY.
 * 
 * @return 
